...
contract AutoAppealableArbitrator is Arbitrator {
     ...
     */
    function arbitrationCost(bytes calldata _extraData) public view override returns(uint fee) {
     ...
     *  @return fee Amount to be paid.
     */
    function appealCost(uint _disputeID, bytes calldata _extraData) public view override returns(uint fee) {
    ...
    function giveRuling(uint _disputeID, uint _ruling) external onlyOwner {
        ...
        payable(msg.sender).transfer(dispute.fees); // Avoid blocking.
     ...
     */
    function appeal(uint _disputeID, bytes calldata _extraData) public override requireAppealFee(_disputeID, _extraData) payable {
        Dispute storage dispute = disputes[_disputeID];
        require(dispute.status == DisputeStatus.Appealable, "The dispute must be appealable.");
